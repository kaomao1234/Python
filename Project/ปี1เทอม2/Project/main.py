...
def on_click(e):
    if e.widget['text'] == 'Back':
        App.all_page[App.index].pack_forget()
        App.index -= 1
        App.all_page[App.index].pack()
    elif e.widget['text'] == 'Sign up':
        SignUp().run


def command_on_click():
    get_user = list(App.LoginPage._Page1__check_user.keys())[0]
    get_password = App.LoginPage._Page1__check_user[get_user]
...
class SetTime:
    ...
    def submit_click(self, e):
        ...
        if check_drug(drug_name, disease_name, drug_quantity) == True:
            ...
            App.PutInfoPage.list_time.insert(END, f"{self.time_set}")
...
class App:
    def __init__(self):
        self.root = Tk()
        self.root.title('Healthy Project')
        self.root.minsize(600, 500)
        self.root.maxsize(600, 500)
        self.LoginPage = Page1(self.root)
        self.PutInfoPage = Page2(self.root)
        self.index = 0
    @property
    def switch(self):
        self.all_page = [self.LoginPage.run, self.PutInfoPage.run]
        self.all_page[self.index].pack() 
App = App()
App.switch
App.root.mainloop()
