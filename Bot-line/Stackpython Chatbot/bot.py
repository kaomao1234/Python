...
cred = credentials.Certificate("botkaomao-firebase-adminsdk-8c6ws-54005cc9cf.json")
...
def MainFunction():
    ...
    # pprint.pprint(dict(question_from_dailogflow_raw))
    pprint.pprint(dict(question_from_dailogflow_raw))
...
def generating_answer(question_from_dailogflow_dict):
    ...
    pprint.pprint(intent_group_question_str)
    #Select function for answering question
    if intent_group_question_str == 'หิวโว้ย':
        answer_str = menu_recormentation()
    elif intent_group_question_str == 'อ้วน - custom - yes':
